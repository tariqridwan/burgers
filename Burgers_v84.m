...
C_k = 0.4523;
c1 = 0.42;  % courant coeff
tmax = 230; % total time
% C_k = 0.05;
% c1 = 0.073;  % courant coeff
% tmax = 900; % total time
dt = c1 * Re / N^2;
total_time = dt*tmax;
m = 2;
...
%         nu_eff_1(k) = 1/Re; % for normal & DNS
        Ri_1(k) = -up_iq_uq - k^2 * nu_eff_1(k) * ui_1(k);
...
%         vt_star(k) = 1 + 34.5*exp(1)^(-3.03*(N/k)); % NO NEED TO REPEAT
        nu_t_2(k) = nut_inf*sqrt( ui_2(N)*conj(ui_2(N))/N )*vt_star(k);
        nu_eff_2(k) = 1/Re + nu_t_2(k); % for LES
%         nu_eff_2(k) = 1/Re; % for normal & DNS
        Ri_2(k) = -up_iq_uq - k^2 * nu_eff_2(k) * ui_2(k);
...
%         vt_star(k) = 1 + 34.5*exp(1)^(-3.03*(N/k)); % NO NEED TO REPEAT
        nu_t_3(k) = nut_inf*sqrt( ui_3(N)*conj(ui_3(N))/N )*vt_star(k);
        nu_eff_3(k) = 1/Re + nu_t_3(k); % for LES
%         nu_eff_3(k) = 1/Re; % for normal & DNS
        Ri_3(k) = -up_iq_uq - k^2 * nu_eff_3(k) * ui_3(k); % RK 4th stage ends
...
legend_plot = sprintf('$u$, when $N=%d$ (LES), $C_k=0.4523$',N);
legend(legend_plot,'Interpreter','latex','fontsize',11)
